...
local function create_button(symbol, color, hover_color, cmd, key)
    local icon = wibox.widget {
        ...
        align = "center",
        valign = "center",
        font = "JetBrains Mono Nerd Font 50",
...
-- Create app buttons
local browser    = create_button("", x.color3, x.color11, apps.browser,      "c")
local discord    = create_button("", x.color5, x.color13, apps.discord,      "d")
local wechat     = create_button("", x.color5, x.color13, apps.wechat,       "w")
...
local music      = create_button("", x.color5, x.color13, apps.music,        "M")
...
local bilibili   = create_button("", x.color4, x.color12, apps.bilibili,     "r")
local github     = create_button("", x.color6, x.color14, apps.github,       "l")
local gfw        = create_button("ﳼ", x.color2, x.color10, apps.gfw,          "s")
local editor     = create_button("", x.color2, x.color10, apps.editor,       "v")
...
app_drawer:setup                 {
    ...
    {
        -- Stripes
        create_stripe({ browser,  youtube,    wechat,     telegram        }, "#00000000"),
        create_stripe({ mail,     editor,     files,      passwords       }, x.color8.."20"),
        create_stripe({ music,    bilibili,   github,     gfw             }, x.color8.."40"),
